...
async function retryUpdate() {
  console.log('Retrying failed updates from offset 1500...\n');
  ...
  while (offset < 2140) {
    batchNumber++;
    console.log(`\n${'='.repeat(60)}`);
    console.log(`RETRY BATCH ${batchNumber} - Processing rows ${offset} to ${offset + batchSize}`);
    ...
    try {
      ...
      for (const product of products) {
        ...
        try {
          ...
          console.error(`Network error for product ${product.id}:`, err.message);
          batchErrors++;
        }
      ...
      console.log(`  Total so far: ${offset + products.length}/2140`);
      ...
      console.error(`Batch ${batchNumber} failed with error:`, err.message);
  ...
  console.log(`${'='.repeat(60)}`);
  console.log(`Additional products updated: ${totalUpdated}`);
  console.log(`${'='.repeat(60)}`);
...
retryUpdate().catch(console.error);
